...
import {executeGetBanners, executeGetContent, executeGetContentBySeries} from "../../api/endPoints.jsx";
...
const TvShowsDetail = memo(() => {
  const { selectedTVSeriesContents,selectedTVSeries } = useViewData();
  const { setVideoUrls,setSelectedVideoData, setSelectedVideoArray,  }= useViewData();
  const { t } = useTranslation();
  const [contentData,setContentData]= useState([]);
  const [contentDataBySeries,setContentDataBySeries]= useState([]);
  const [seriesContentData,setSeriesContentData]= useState([]);
  const options1 = [
    { value: `${t('header.playlist')}`, label: `${t('header.playlist')}` },
    { value: `${selectedTVSeries}`, label: `${selectedTVSeries}` },
  ...
  const getContentsBySeries = async () => {
    console.log('content Data Execute start');
    try {
      const response = await executeGetContentBySeries(selectedTVSeriesContents['category']  ? selectedTVSeriesContents['category'] : 'Teledrama' , selectedTVSeries);
  ...
  return (
    <Fragment>
      {console.log('print series data ===>',contentDataBySeries)}
      {console.log('selectedTVSeries  ===>',selectedTVSeries)}
      <div className="tv-show-detail">
        <Container fluid>
          <div
            ...
            style={{
              // background: `url({
              //   selectedTVSeriesContents['thumbnail_url']
              // }`,
            ...
            <div className="banner-caption">
              <div className="trending-info p-0">
                ...
                <div className="ratting-start p-0 m-0 list-inline text-warning d-flex align-items-center justify-content-left">
                  ...
                  <img
                    src={selectedTVSeriesContents['thumbnail_url'] ? selectedTVSeriesContents['thumbnail_url'] :selectedTVSeriesContents['banner_url']}
                    alt="imdb-logo"
